...
abstract class cocheraPDO{
	...
	public static function traerCocheras()
	{
	...
	public static function cocheraDiscapacitadoValidation($nombre)
	{
	    $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
		$consulta = $objetoAccesoDato->RetornarConsulta
		("
		    SELECT ReservadoDiscEmbar FROM `cocheras` where nombre=:nombre
		");
		$consulta->bindValue(':nombre', $nombre, PDO::PARAM_STR);
		$consulta->execute();			
		$queryResponse = $consulta->fetch(PDO::FETCH_ASSOC);
		return $queryResponse["ReservadoDiscEmbar"];	
	}

    public static function traerEstadoOcupadoCochera($id)
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
		$consulta = $objetoAccesoDato->RetornarConsulta
		("
			SELECT ocupada FROM cocheras WHERE id=:id
		");
		$consulta->bindValue(':id', $id, PDO::PARAM_STR);
		$consulta->execute();			
		$queryResponse = $consulta->fetch(PDO::FETCH_ASSOC);
		return $queryResponse["ocupada"];
    }

    public static function toggleOcupadoCochera($id, $estado)
    {
        //UPDATE `cocheras` SET `ocupada`=1 WHERE nombre=102
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
		$consulta = $objetoAccesoDato->RetornarConsulta
		("
		    UPDATE `cocheras` SET `ocupada`=:estado WHERE id=:id
		");
